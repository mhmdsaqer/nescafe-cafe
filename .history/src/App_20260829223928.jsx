...
export default function App() {
  return (
    <div className="min-h-screen bg-coffee-light text-coffee-dark font-sans" dir="rtl">
      ...
      {/* 2. Hero Section - (خلفية كاملة مع نص على اليسار) */}
      <section 
        id="hero" 
        className="pt-24 min-h-screen flex items-center relative bg-cover bg-center bg-no-full"
        style={{ backgroundImage: "url('/hero.jpeg')" }}
      >
        {/* طبقة تظليل تضمن وضوح النص */}
        <div className="absolute inset-0 bg-black/30 backdrop-blur-[2px]"></div>

        <div className="max-w-7xl mx-auto px-6 w-full relative z-10 flex justify-start">
          
          {/* Hero Content - تم توجيهه للجهة اليسرى */}
          <div className="space-y-6 max-w-xl text-right md:text-left md:mr-auto">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-white text-xs font-bold border border-white/20">
              <Sparkles className="w-4 h-4 text-amber-300" /> تجربة نسكافيه فريدة
            </span>
            <h1 className="text-4xl md:text-6xl font-extrabold leading-tight text-white drop-shadow-lg">
              قهوة الصباح.. <br />
              <span className="text-amber-300">طعم لا يُقاوم</span>
            </h1>
            ...
            <div className="flex items-center gap-4 pt-4 justify-start">
              <a href="#menu" className="bg-coffee-brand hover:bg-[#a94911] text-white px-8 py-3.5 rounded-full font-semibold shadow-lg shadow-black/20 transition-all">
                استكشف القائمة
              </a>
              <a href="#about" className="bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/30 text-white px-6 py-3.5 rounded-full font-semibold transition-all">
                تعرف علينا
              </a>
            </div>
          </div>
